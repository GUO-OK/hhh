...
def result_page(page, search_params, on_back=None, username=None, is_logged_in=False):
    print(f"result_page 接收参数: username={username}, is_logged_in={is_logged_in}")
    ...
    def toggle_favorite(route_id, btn, route_name):
        print(f"点击收藏: route_id={route_id}, route_name={route_name}")  # 调试
        if not is_logged_in:
                ...
                ft.Container(
                    ...
                    content=ft.Column(
                        controls=[
                            ...
                            ft.Text(f"🏷️ 你是我的它个斯~：{row.get('tags', '')}",
                                    size=SEARCH_STYLES["result_text"]["detail_size"],
                                    color=SEARCH_STYLES["result_text"]["detail_color"]),
